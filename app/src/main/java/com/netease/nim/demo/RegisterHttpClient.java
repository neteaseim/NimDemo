...
public class RegisterHttpClient {
    ...
    public void register(String account, String nickName, String password, final HttpCallback<Void> callback) {
        ...
        NimHttpClient.getInstance().execute(REGISTER_URL, headers, bodyString, new NimHttpClient.NimHttpCallback() {
            @Override
    ...
    private String readAppKey() {
        try {
            ApplicationInfo appInfo = NimUIKit.getContext().getPackageManager()
                    .getApplicationInfo(NimUIKit.getContext().getPackageName(), PackageManager.GET_META_DATA);
            if (appInfo != null) {
